...
def cleanData(df, ):
    ...
    print("eliminated rows without store-id is empty")
    ...
    print("extract year, month and day from Date")
    ...
    def helper_schoolholiday(row):
        for i in range(len(df)):
            if pd.isnull(df.row['SchoolHoliday']):
                return 0.0
            else:
                return row['SchoolHoliday']

    df['SchoolHoliday'] = df.apply(helper_schoolholiday, axis=1)
    print("fill school holiday based o state holiday")
...
# take care of regional stateholiday
    ...
    print('finish regional stateholiday')
...
store = promoInterval(store)

def fillEmptyDistances(row):
    """FIlling empty distances with mean"""
    if pd.isnull(row['CompetitionDistance']):
        return store['CompetitionDistance'].mean()
    else:
        return row['CompetitionDistance']

store['CompetitionDistance'] = store.apply(fillEmptyDistances, axis=1)

def encodeStoreType(store):
    """Gets dummies for 'StoreType' into four columns and concat them to the table"""
    NewStoreType = pd.get_dummies(store['StoreType'])
    NewStoreType.rename(columns={'a': 'StoreType a', 'b': 'StoreType b', 'c': 'StoreType c', 'd': 'StoreType d'},
                        inplace=True)
    store = pd.concat([store, NewStoreType], axis=1)
    print('Store Type Encoded')
    return store

store = encodeStoreType(store)

def encodeAssortment(store):
    ...
    store = pd.concat([store, NewAssortment], axis=1)
    print('Assortment Type Encoded')
    return store

store = encodeAssortment(store)